...
export class VanillaGameProfileRegistry {
  ...
  public isPokemonAllowed(format: string, pokemon: PokemonData): boolean {
    ...
    const hasExplicitPool = Boolean(profile.allowedDexNumbers?.length || profile.allowedDexRanges?.length);

    if (profile.allowedDexNumbers?.includes(dexNumber)) return true;

    if (profile.allowedDexRanges?.length) {
      const isInRange = profile.allowedDexRanges.some(range => dexNumber >= range.from && dexNumber <= range.to);

      if (isInRange) return true;
    }

    if (hasExplicitPool) return false;
...
const HISUI_POKEDEX_NUMBERS = [
  ...
  484, 485, 486, 487, 488, 489, 490, 491, 492, 493, 501, 502, 503, 548,
  549, 550, 570, 571, 627, 628, 641, 642, 645, 700, 704, 705, 706, 712,
  713, 722, 723, 724, 899, 900, 901, 902, 903, 904, 905,
];
...
export const VANILLA_GAME_PROFILES: Record<string, VanillaGameProfile> = {
  ...
  vanilla_legends_arceus: profile({
    ...
    poolStatus: 'bootstrap',
    allowedDexNumbers: HISUI_POKEDEX_NUMBERS,
    strictPool: true,
    allowMegas: false,
    allowRegionalForms: true,
